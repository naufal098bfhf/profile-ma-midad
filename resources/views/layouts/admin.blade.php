...
<body class="{{ $routeClass }}{{ $extraClass }}">
   ...
   class="sidebar-brand d-flex align-items-center">
    ...
    <span class="ms-2 fw-bold"
        ...
        MA Miftahul Midad
			...
			<div class="sidebar-body">
				<ul class="nav">
					...
					@php
						$ppdbActive = request()->is('admin/ppdb*');
					@endphp
					<li class="nav-item">
						<a href="{{ route('admin.ppdb.index') }}" class="nav-link {{ $ppdbActive ? 'active' : '' }}">
							<i class="link-icon" data-feather="users"></i>
							<span class="link-title">PPDB</span>
						</a>
					</li>
					<li class="nav-item">
		...
		<div class="page-wrapper">
			...
			<footer class="footer d-flex flex-column flex-md-row align-items-center justify-content-between px-4 py-3 border-top small">
				<p class="text-muted mb-1 mb-md-0">Copyright © 2026 <a href="{{ route('home') }}" target="_blank">Naufal Hibatullah</a>.</p>
	...
	<script>
		...
		window.addEventListener('load', function() {
			try {
				...
				console.warn('Feather Icons fallback failed:', error);
			}
		});
	</script>
	<script>
		// Load template.js with error handling
		(function() {
			try {
				var script = document.createElement('script');
				script.src = "{{ asset('template/assets/js/template.js') }}";
				script.onerror = function() {
					console.warn('Template.js failed to load');
				};
				document.head.appendChild(script);
			} catch (error) {
				console.warn('Error loading template.js:', error);
			}
		})();
	</script>
	<!-- endinject -->
	...
	<script>
		// Configure toastr with error handling
		document.addEventListener('DOMContentLoaded', function() {
			try {
				...
				console.warn('Toastr configuration failed:', error);
			}
		});
	</script>
	<!-- End Toastr JS -->
